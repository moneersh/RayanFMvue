<template>
  <header
    class="fixed top-0 left-0 right-0 z-50 bg-rayan-light/95 backdrop-blur-lg border-b border-rayan-gray/50 shadow-lg"
    :class="{ 'rtl-header': locale.value === 'ar' }"
  >
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-20 lg:h-24">
        ...
        <div class="flex-shrink-0 flex items-center">
          <router-link
            ...
            :class="{ 'space-x-reverse': locale.value === 'ar' }"
          >
            <!-- Rayan FM Logo -->
            <div class="relative w-16 h-16 lg:w-20 lg:h-20 group-hover:scale-105 transition-transform duration-300">
              ...
              <div class="relative w-full h-full rounded-2xl overflow-hidden shadow-lg ring-2 ring-rayan-orange/30 group-hover:ring-rayan-orange/50 transition-all duration-300">
                ...
                <div
                  v-if="logoError"
                  class="absolute inset-0 rounded-full bg-gradient-to-br from-rayan-purple to-rayan-purple/80 shadow-lg flex items-center justify-center"
                >
              ...
              <div class="absolute inset-0 rounded-full border-2 border-rayan-orange/30 animate-ping" />
            </div>
            ...
            <div class="flex flex-col">
              <h1 class="text-2xl lg:text-3xl font-black bg-gradient-to-r from-rayan-orange to-rayan-purple bg-clip-text text-transparent leading-tight">
                {{ locale.value === 'ar' ? 'راديو الريان' : 'Rayan FM' }}
              </h1>
              <div
              ...
              >
                ...
                <div class="w-2 h-2 bg-rayan-orange rounded-full animate-pulse" />
              </div>
            </div>
          </router-link>
        </div>
        ...
        >
          ...
          >
            {{ locale.value === 'ar' ? item.nameAr : item.nameEn }}
            <span class="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-rayan-orange to-rayan-purple group-hover:w-full transition-all duration-300" />
          </router-link>
        </nav>
        ...
        >
          ...
          <div class="hidden sm:flex items-center bg-rayan-orange/10 text-rayan-orange px-4 py-2 rounded-full text-sm font-bold border border-rayan-orange/20">
            <div
              class="w-2 h-2 bg-rayan-orange rounded-full mr-2 animate-pulse"
              :class="{ 'ml-2 mr-0': locale.value === 'ar' }"
            />
            {{ locale.value === 'ar' ? 'مباشر' : 'LIVE' }}
          </div>
      ...
      >
        <div
          v-if="isMobileMenuOpen"
          class="lg:hidden border-t border-rayan-gray/50 bg-rayan-light/95 backdrop-blur-lg"
        >
          <div class="px-4 py-6 space-y-4">
            ...
            <div class="space-y-2">
              ...
              >
                <span class="font-medium">{{ locale.value === 'ar' ? item.nameAr : item.nameEn }}</span>
                <svg
                  class="w-5 h-5 text-rayan-gray group-hover:text-rayan-orange transition-colors duration-300"
                  :class="{ 'rotate-180': locale.value === 'ar' }"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center bg-rayan-orange/10 text-rayan-orange px-4 py-2 rounded-xl text-sm font-bold border border-rayan-orange/20">
                  <div
                    ...
                    :class="{ 'ml-2 mr-0': locale.value === 'ar' }"
                  />
                  {{ locale.value === 'ar' ? 'البث المباشر' : 'LIVE NOW' }}
                </div>
